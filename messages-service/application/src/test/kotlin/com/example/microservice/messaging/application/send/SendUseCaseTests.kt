...
class SendUseCaseTests {
    ...
    fun testSetup() {
        ...
        every { messageMapper.convertToMessage(any()) }.returns(Message("test"))
        every { messageMapper.convertToMessageDbObj(any()) }.returns(PersistentMessage())
    ...
    inner class SendMessageTests {
        ...
        fun `should call the message entity's send function`() {
            ...
            every { messageMapper.convertToMessage(any()) }.returns(message)
        ...
        fun `should save the message to a store`() {
            ...
            every { message.send() }.answers { }
            every { messageMapper.convertToMessage(any()) }.returns(message)
            every { messageMapper.convertToMessageDbObj(any()) }.returns(dbObject)
